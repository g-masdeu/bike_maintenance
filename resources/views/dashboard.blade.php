    ...
    <script>
                ...
                updateForm.addEventListener('submit', (e) => {
                    ...
                    fetch(`/api/bicicletas/${id}`, {
                        ...
                        .then(data => {
                            ...
                            // Comprobem si s'ha de fer algun manteniment
                            fetch(`/api/mantenimientos/check/${data.id}`, {
                                    method: 'PUT',
                                    headers: {
                                        'Accept': 'application/json'
                                    },
                                ...
                                .then(res => res.json())
                                .then(result => {
                                    console.log('Resultado mantenimiento:', result);

                                    const necesarios = result.mantenimientos.filter(m => m
                                        .necesita);
                                    if (necesarios.length > 0) {
                                        alert(
                                            `⚠️ La bicicleta necesita mantenimiento:\n${necesarios.map(m => '- ' + m.mantenimiento).join('\n')}`
                                            );
                                    }
        ...
        `;
                        ...
                        .catch(err => {
                            console.error(err);
                            detalle.innerHTML =
                                '<p class="text-red-500">Error cargando los datos</p>';
                        });
                });
            });
                ...
                @foreach ($bicicletas as $bicicleta)
                    <div class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow flex justify-between items-center">
                        <div>
                            ...
                            <div class="text-sm text-gray-500 dark:text-gray-400">{{ $bicicleta->kms_actuals }} km
                                recorridos</div>
                        </div>
                        <div class="flex flex-col sm:flex-row gap-2 sm:gap-3">
                            ...
                            <button
                                ...
                                data-id="{{ $bicicleta->id }}" data-kms="{{ $bicicleta->kms_actuals }}">
                                Actualizar km
                            </button>
                        </div>
                    </div>
